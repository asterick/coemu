...
(function () {
    function HMU1440(data) {
        this.sectors = new Array(this.SECTOR_COUNT);
        
        data = data || [];
        
        for (var i = 0, idx = 0; i < this.sectors.length; i++) {
            this.sectors[i] = new Uint16Array(this.SECTOR_COUNT);
            for (var p = 0; p < this.SECTOR_LENGTH; p++, idx++ ) {
                this.sectors[i][p] = (idx < data.length) ? data[idx] : 0;
            }
        }
    }
    
    Object.defineProperties(HMU1440.prototype, {
        "SECTOR_COUNT": { value: 1440 },
        "SECTOR_LENGTH": { value: 512 },
        "QUALITY_LEVEL": { value: 0x7FFF }
    });
    
    this.HMU1440 = HMU1440;
}).call(this);